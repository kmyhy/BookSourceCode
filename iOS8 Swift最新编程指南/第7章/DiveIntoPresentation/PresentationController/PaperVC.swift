...
class PaperVC: UIViewController,UIPopoverPresentationControllerDelegate{
    var transitionDelegate : MyTransitioningDelegate?
    ...
    @IBAction func quizAnswerButtonPressed(sender: UIButton) {
        ...
        rect.origin = origin
        // 2
        let selectedItem = SelectedItem(snapshot: sender.pb_takeSnapshot(),
            originRect: rect)
        // 3
        transitionDelegate = MyTransitioningDelegate(selectedItem: selectedItem)
        transitioningDelegate = transitionDelegate
        
        
        var myAlert: MyAlertVC = storyboard?.instantiateViewControllerWithIdentifier("idMyAlert") as MyAlertVC
        
        myAlert.message = message

        myAlert.modalPresentationStyle = UIModalPresentationStyle.Custom
        myAlert.transitioningDelegate = transitionDelegate;
